package Toe;
import java.util.Random;
import java.util.Scanner;
public class TicTacToe {
	 private static char[][] board = {{' ', ' ', ' '}, {' ', ' ', ' '}, {' ', ' ', ' '}};
	    private static char currentPlayer;
	    private static boolean isGameOver;

	    public static void main(String[] args) {
	        Scanner scanner = new Scanner(System.in);

	        // Decide if player is X or O
	        System.out.print("Choose your marker (X or O): ");
	        char playerMarker = scanner.next().toUpperCase().charAt(0);
	        currentPlayer = playerMarker;

	        // Decide if playing against another player or computer
	        System.out.print("Play against a friend (Y/N)? ");
	        char choice = scanner.next().toUpperCase().charAt(0);

	        if (choice == 'Y') {
	            playAgainstFriend(scanner);
	        } else {
	            playAgainstComputer(scanner);
	        }

	        scanner.close();
	    }

	    private static void playAgainstFriend(Scanner scanner) {
	        while (!isGameOver) {
	            printBoard();
	            makeMove(scanner);
	            checkGameStatus();
	            switchPlayer();
	        }
	        printBoard();
	        printResult();
	    }

	    private static void playAgainstComputer(Scanner scanner) {
	        while (!isGameOver) {
	            printBoard();

	            if (currentPlayer == 'X') {
	                makeMove(scanner);
	            } else {
	                makeComputerMove();
	            }

	            checkGameStatus();
	            switchPlayer();
	        }
	        printBoard();
	        printResult();
	    }

	    private static void makeMove(Scanner scanner) {
	        int row, col;
	        do {
	            System.out.print("Enter your move (row[1-3] column[1-3]): ");
	            row = scanner.nextInt() - 1;
	            col = scanner.nextInt() - 1;
	        } while (!isValidMove(row, col));

	        board[row][col] = currentPlayer;
	    }

	    private static boolean isValidMove(int row, int col) {
	        if (row < 0 || row >= 3 || col < 0 || col >= 3 || board[row][col] != ' ') {
	            System.out.println("Invalid move. Try again.");
	            return false;
	        }
	        return true;
	    }

	    private static void makeComputerMove() {
	        Random random = new Random();
	        int row, col;
	        do {
	            row = random.nextInt(3);
	            col = random.nextInt(3);
	        } while (!isValidMove(row, col));

	        System.out.println("Computer's move: " + (row + 1) + ", " + (col + 1));
	        board[row][col] = currentPlayer;
	    }

	    private static void printBoard() {
	        System.out.println("-------------");
	        for (int i = 0; i < 3; i++) {
	            System.out.print("| ");
	            for (int j = 0; j < 3; j++) {
	                System.out.print(board[i][j] + " | ");
	            }
	            System.out.println("\n-------------");
	        }
	    }

	    private static void checkGameStatus() {
	        if (checkWin()) {
	            isGameOver = true;
	        } else if (checkDraw()) {
	            isGameOver = true;
	        }
	    }

	    private static boolean checkWin() {
	        // Check rows, columns, and diagonals for a win
	        return (checkRows() || checkColumns() || checkDiagonals());
	    }

	    private static boolean checkRows() {
	        for (int i = 0; i < 3; i++) {
	            if (board[i][0] == currentPlayer && board[i][1] == currentPlayer && board[i][2] == currentPlayer) {
	                return true;
	            }
	        }
	        return false;
	    }

	    private static boolean checkColumns() {
	        for (int i = 0; i < 3; i++) {
	            if (board[0][i] == currentPlayer && board[1][i] == currentPlayer && board[2][i] == currentPlayer) {
	                return true;
	            }
	        }
	        return false;
	    }

	    private static boolean checkDiagonals() {
	        return (board[0][0] == currentPlayer && board[1][1] == currentPlayer && board[2][2] == currentPlayer) ||
	               (board[0][2] == currentPlayer && board[1][1] == currentPlayer && board[2][0] == currentPlayer);
	    }

	    private static boolean checkDraw() {
	        for (int i = 0; i < 3; i++) {
	            for (int j = 0; j < 3; j++) {
	                if (board[i][j] == ' ') {
	                    return false;
	                }
	            }
	        }
	        System.out.println("The game is a draw!");
	        return true;
	    }

	    private static void switchPlayer() {
	        currentPlayer = (currentPlayer == 'X') ? 'O' : 'X';
	    }

	    private static void printResult() {
	        if (checkWin()) {
	            switchPlayer();
	            System.out.println("Player " + currentPlayer + " wins!");
	        } else if (checkDraw()) {
	            System.out.println("It's a draw!");
	        }
	    }
}
